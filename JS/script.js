...
$(document).ready(function(){
  ...
  setInterval(() => {
    ...
    Math.floor(clock.getTime().time % 60) < 10 ?  globalThis.min = "0"+Math.floor(clock.getTime().time % 60) :  globalThis.min = Math.floor(clock.getTime().time % 60) 
    Math.floor(clock.getTime().time / 60) < 10 ?  globalThis.sec = "0"+Math.floor(clock.getTime().time / 60) :  globalThis.sec = Math.floor(clock.getTime().time / 60) 
    ...
    if(document.getElementById("stats").innerHTML != "Hey!") // timer clock running
    {
        if(defaultClkType != "timer") //another mode working
        {
            if ((document.getElementById("st-sc").innerHTML == "00") || (document.getElementById("st-ms").innerHTML == "000")) //stop watch not working
            {
                ...
                document.getElementById("mn_nav_ct").innerHTML = sec;
                document.getElementById("sc_nav_ct").innerHTML = min;
            }
            else
            {
                document.getElementById("navBarSt").style.top = "50%";
...
else
{
  ...
  countS = localStorage.getItem("currentSession");
}
